...
from src import common_ops, interface
...
def _get_port_v1(port_name, depth=0, selector=None, **kwargs):
    ...
    if not common_ops._response_ok(response, "GET"):
        print("FAIL: Getting Port table entry '%s' failed with status code %d" % (port_name, response.status_code))
        output = {}
    else:
...
def get_all_ports(**kwargs):
    ...
    if not common_ops._response_ok(response, "GET"):
        print("FAIL: Getting list of all Port table entries failed with status code %d"
              % response.status_code)
    else:
        print("SUCCESS: Getting list of all Port table entries succeeded")
...
def add_vlan_port(vlan_port_name, vlan_id, ipv4=None, vrf_name="default", vlan_port_desc=None,
    ...
    if "/rest/v1/system/ports/%s" % vlan_port_name not in ports_list:
        ...
        if not common_ops._response_ok(response, "POST"):
            print("FAIL: Adding Port table entry '%s' for SVI failed with status code %d"
                  % (vlan_port_name, response.status_code))
        else:
            print("SUCCESS: Adding Port table entry '%s' for SVI succeeded" % vlan_port_name)
    else:
        print("SUCCESS: No need to create VLAN Port '%s' since it already exists" % vlan_port_name)
...
def add_l2_port(port_name, port_desc=None, port_admin_state="up", **kwargs):
    ...
    if "/rest/v1/system/ports/%s" % port_name_percents not in ports_list:
        ...
        if not common_ops._response_ok(response, "POST"):
            ...
            print("SUCCESS: Adding Port table entry '%s' succeeded" % port_name)
    else:
        target_url = kwargs["url"] + "system/ports/%s" % port_name_percents
        ...
        if not common_ops._response_ok(response, "PUT"):
            print("FAIL: Updating Port table entry '%s' failed with status code %d"
                  % (port_name, response.status_code))
        else:
            print("SUCCESS: Updating Port table entry '%s' succeeded" % port_name)
...
def add_l3_ipv4_port(port_name, ip_address=None, port_desc=None, port_admin_state="up", vrf="default", **kwargs):
    ...
    if "/rest/v1/system/ports/%s" % port_name_percents not in ports_list:
        ...
        if not common_ops._response_ok(response, "POST"):
            print("FAIL: Adding IPv4 Port table entry '%s' failed with status code %d"
                  % (port_name, response.status_code))
        else:
            print("SUCCESS: Adding IPv4 Port table entry '%s' succeeded" % port_name)
    else:
        update_port_ipv4(port_name, ip_address, port_admin_state, vrf, **kwargs)
...
def add_l3_ipv6_port(port_name, ip_address=None, port_desc=None, port_admin_state="up", vrf="default", **kwargs):
    ...
    if "/rest/v1/system/ports/%s" % port_name_percents not in ports_list:
        ...
        if not common_ops._response_ok(response, "POST"):
            print("FAIL: Adding L3 IPv6 Port table entry '%s' failed with status code %d"
                  % (port_name, response.status_code))
        else:
            print("SUCCESS: Adding L3 IPv6 Port table entry '%s' succeeded" % port_name)
            # IPv6 defaults
            ipv6_data = {
                "address": ip_address,
                "node_address": True,
            ...
            if not common_ops._response_ok(response, "POST"):
                print("FAIL: Final configuration of L3 IPv6 Port table entry '%s' failed with status code %d"
                      % (port_name, response.status_code))
            else:
                print("SUCCESS: Final configuration of L3 IPv6 Port table entry '%s' succeeded" % port_name)
    else:
        port_data.pop('name', None)  # must remove this item from the json since name can't be modified
        target_url = kwargs["url"] + "system/ports/%s" % port_name_percents
        payload_data = json.dumps(port_data, sort_keys=True, indent=4)
        ...
        if not common_ops._response_ok(response, "PUT"):
            print("FAIL: Updating L3 IPv6 Port table entry '%s' failed with status code %d"
                  % (port_name, response.status_code))
        else:
            print("SUCCESS: Updating L3 IPv6 Port table entry '%s' succeeded" % port_name)
...
def update_port_ipv6(port_name, ip_address, addr_type="global-unicast", **kwargs):
    ...
    if not common_ops._response_ok(response, "POST"):
        print("FAIL: Creating IPv6 address to update Port '%s' to '%s' failed with status code %d"
              % (port_name, ip_address, response.status_code))
    else:
        print("SUCCESS: Creating IPv6 address to update Port '%s' to '%s' succeeded"
...
def delete_port(port_name, **kwargs):
    ...
    if "/rest/v1/system/ports/%s" % port_name_percents in ports_list:
        ...
        if not common_ops._response_ok(response, "DELETE"):
            print("FAIL: Deleting Port '%s' failed with status code %d" % (port_name, response.status_code))
        else:
            print("SUCCESS: Deleting Port '%s' succeeded" % port_name)
    else:
        print("SUCCESS: No need to remove  Port '%s' since it doesn't exist" % port_name)
...
def _delete_ipv6_address(port_name, ip, **kwargs):
    ...
    if ip in interface.get_ipv6_addresses(port_name, **kwargs):
        ...
        if not common_ops._response_ok(response, "DELETE"):
            print("FAIL: Deleting IPv6 Address '%s' from Port table entry '%s' failed with status code %d"
                  % (ip, port_name, response.status_code))
        else:
            print("SUCCESS: Deleting IPv6 Address '%s' from Port table entry '%s' succeeded"
                  % (ip, port_name))
    else:
        print("SUCCESS: No need to delete IPv6 Address '%s' from Port table entry '%s' since it does not exist"
              % (ip, port_name))
    ...
    if "/rest/v1/system/ports/%s" % port_name not in ports_list:
        ...
        if not common_ops._response_ok(response, "POST"):
            ...
            if not common_ops._response_ok(response, "POST"):
                ...
                print("SUCCESS: Adding Port and Interface table entries '%s' succeeded" % port_name)

    else:
        update_port_ipv4(port_name, ipv4, "up", vrf, **kwargs)
    ...
    if "/rest/v1/system/ports/%s" % port_name not in ports_list:
        ...
        if not common_ops._response_ok(response, "POST"):
            ...
            if not common_ops._response_ok(response, "POST"):
                ...
                print("SUCCESS: Adding VXLAN Port and Interface table entries '%s' succeeded" % port_name)
    else:
        update_port_ipv4(port_name, source_ipv4, "up", "default", **kwargs)
    ...
    if port_data:
        ...
        if not common_ops._response_ok(response, "PUT"):
            print("FAIL: Updating Port '%s' with admin-configured state '%s' "
                  "failed with status code %d" % (port_name, state, response.status_code))
        else:
            print("SUCCESS: Updating Port '%s' with admin-configured state '%s' "
                  "succeeded" % (port_name, state))
    else:
        print("Unable to update Port '%s' because operation could not find existing Port" % port_name)
...
def initialize_port_entry(port_name, **kwargs):
    ...
    if not common_ops._response_ok(response, "PUT"):
        print("FAIL: Initializing port '%s' failed with status code %d" % (port_name, response.status_code))
    else:
